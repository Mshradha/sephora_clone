var RemoverData=JSON.parse(localStorage.getItem("makeOver"));
    var productArr=[] ;
    showdata(RemoverData);
    
    function filterCat(){
      var selected = document.querySelector("#brandFilter").value;

      var filterdList =RemoverData.filter(function(elem){
    ...
    var selected = document.querySelector("#priceSort").value;
     ...
     if (selected == "low"){
      ...
      console.log(RemoverData);
      showdata(RemoverData);

    }


    function showdata(RemoverData){
        document.getElementById("containershra").innerHTML="";

       RemoverData.map(function(el){
